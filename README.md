...
```
  (scr <- system.file("scripts", package = "trajclass"))
  list.files(scr, pattern = "[[:alnum:]][.][R]$")
...
`step1.R` script generates an external file: `step1_ objects.rda` required by `step2.R` and `step3.R`
